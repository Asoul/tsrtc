...
function loadStockInfo(stock,d0) {
    ...
    $.getJSON(apiBaseUrl+"getStockInfo.jsp?ex_ch="+stock+"&json=1&delay=0", function(data) {
        if(data.rtcode=="0000") {
...
function loadChart(channel,d0,upper,lower,yesPrice) {
    ...
    $.getJSON(apiBaseUrl+"getOhlc.jsp?ex_ch="+channel+"&d0="+d0, function(data) {
        var ohlc = [], volume = [], cPrice=[], dataLength = data.msgArray.length;
        var maxValume=0;
